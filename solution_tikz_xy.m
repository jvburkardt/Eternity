function solution_tikz_xy ( x, filename, grid_word, tile_num, tile_word, ...
  plot_scale )

%*****************************************************************************80
%
%% solution_tikz_xy() makes a tikz() image of a tiling solution.
%
%  Discussion:
%
%    The solution is represented by a binary vector of 0 and 1 values.
%
...
%
  ...
  fprintf ( 1, '\n' );
  fprintf ( 1, 'solution_tikz_xy():\n' );
  fprintf ( 1, '  MATLAB/Octave version %s\n', version ( ) );
  fprintf ( 1, '  Make a tikz() image of a tiling solution.\n' ); 
  fprintf ( 1, '  The (x,y) coordinate system is used.\n' );
...
%
  ...
  pxy_r = zeros ( 1, 2 );
  pxy_r(1,1) = pij_r(1,1) * 0.25;
  pxy_r(1,2) = pij_r(1,2) * sqrt ( 3.0 ) / 12.0;

  exy = word_to_edge_xy ( w_r, pxy_r );
  en = size ( exy, 1 );
...
%
  ...
  wxy_r = node_ij_to_xy ( wij_r, pxy_r );
...
%
            if ( inside )
              ...
              if ( x(var_num) == 1 )
                ...
                pxy_s = zeros ( 1, 2 );
                pxy_s(1,1) = pij_s(1,1) * 0.25;
                pxy_s(1,2) = pij_s(1,2) * sqrt ( 3.0 ) / 12.0;

                fprintf ( 1, '  Tile = %d, VAR = %d,', s, var_num );
                fprintf ( 1, '  Rotate = %d, reflect = %d,', r, reflect );
                fprintf ( 1, '  type = %d  pij = (%d,%d)  pxy=(%g,%g)\n', ...
                  key, pij_s, pxy_s );

                exy = word_to_edge_xy ( w_s, pxy_s );
                en = size ( exy, 1 );
